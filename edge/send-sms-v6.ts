/* =============================================================================
   send-sms v6 — R87 (dead RPC call removed; see the R87 note in the handler)
   Copied VERBATIM from send-sms v5 except the `queue_appointment_sms` call.
   ============================================================================= */
/* =============================================================================
...
Deno.serve(async (req: Request) => {
  ...
  /* R87 · CTO — `queue_appointment_sms` never existed in the live database; v5 called it on every
     run and discarded the error (panel 07 #7 / verifier). The Settings switch that implied it
     (`auto_sms_appointment`) is gone this round too. `appointment_queued` stays in the shape, as 0. */
  const results: any = { appointment_queued: 0, sent: 0, failed: 0 };
