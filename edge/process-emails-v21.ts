/* =============================================================================
   process-emails v21 (R83 · fixH) — v20 plus the review fixes. Wording of every
   template is byte-identical to v20 for well-formed data; only escaping, gating and
   the send/stamp bookkeeping changed.

     1. OPT-OUT HONOURS BOTH FLAGS. clients has two opt-out booleans in production:
        comms_optout (set by the unsubscribe link) and marketing_opt_out (the
        "Marketing opt-out" select staff set on the client record). v19/v20 read only
        the first, so a staff-recorded opt-out was not enforced at send time. Both are
        read now (joined row, then the fresh re-read), either one cancels the row.
     2. ESCAPING. Every client / lead / case / adviser / firm-settings value that is
        interpolated into HTML goes through escapeHtml(): first name (which for a
        lead_ack is the FIRST TOKEN OF A PUBLIC WEB-FORM FIELD), lender, rate type,
        rate percent, adviser name, adviser phone, company name, docs_list items and
        the review link. The adviser-written `custom` body is unchanged — the app
        escapes it before it is stored (R66) and it is documented as such.
     4. IDEMPOTENT SEND + RETRY-ABLE FAILURES. The Resend call carries an
        Idempotency-Key derived from the queue row id, so a retry of a row whose first
        attempt actually left cannot produce a second copy. A 429 or 5xx from Resend
        re-queues the row (status 'queued', claimed_at cleared, the reason in `error`)
        instead of marking it permanently failed. The post-send stamping (status
        'sent', fee/fact-find/doc-token/lead clocks) runs OUTSIDE the failure catch:
        once Resend has accepted, a DB hiccup can no longer flip the row to 'failed'
        (which invited a resend of mail that had already gone).
     6. NO SILENT EMPTY RUNS. Errors from the candidate query, the claim and the
        queue_comms_extras RPC are surfaced in the response (500 for the first two,
        `comms_error` for the RPC) instead of being read as "nothing to send".
     7. CHASE / REMINDER WORDING. The "still waiting on" docs chase and the "one
        gentle reminder" review copy are chosen only when a prior row of that type was
        actually SENT — not merely queued or failed.
     8. GATE VALUE NORMALISED. settings.financial_promotions_approved is trimmed and
        lower-cased before the comparison, matching the admin app's finPromosApproved().
     +  List-Unsubscribe / List-Unsubscribe-Post headers on marketing-type sends,
        pointing at the same unsubscribe URL as the footer (unsubscribe v2 accepts POST).
   Finding numbers refer to the R83 review of v20; 3 and 5 live in unsubscribe-v2.ts
   and db/check-schema-drift.js respectively.
   ========================================================================== */
...
function npsScale(caseId: string, tok: string) {
  ...
  const btns = opts.map(([n, label]) =>
    `<a href="${FN_BASE}/nps-capture?case=${escapeHtml(String(caseId))}&score=${n}&token=${escapeHtml(String(tok))}" style="display:inline-block;margin:4px;padding:10px 14px;background:#f3f4f6;border:1px solid #e5e7eb;border-radius:8px;color:#0f2a4a;text-decoration:none;font-size:14px;">${label}</a>`
  ).join("");
...
function compose(type: string, c: any, cl: any, s: Record<string, string>, adv: any, extra: any = {}) {
  /* v21 (fix 2): every value below is HTML-escaped ONCE, here, and only the escaped form is
     used in the templates. `first` for a lead_ack is the first token of leads.name — a public
     web-form field — and the enquirer also chooses the To: address, so an unescaped name was
     attacker-authored HTML leaving on the firm's verified domain. */
  const companyText = s.company_name || "NexMoney";   // v21: plain-text form, for Subject: lines only
  const company = escapeHtml(companyText);
  const adviser = escapeHtml((adv && adv.full_name) || s.adviser_name || "NexMoney");
  // Per-adviser phone falls back to the firm-wide number.
  const advPhone = escapeHtml((adv && adv.phone) || s.adviser_phone || "");
  const phone = advPhone ? ` or call ${advPhone}` : "";
  const first = escapeHtml((cl.first_name || "").trim() || (cl.last_name || "").trim());
  const lender = c && c.lender ? escapeHtml(String(c.lender)) : "";
  const rateType = escapeHtml((c && c.rate_type) || "fixed");
  const ratePct = c && c.rate_percent ? escapeHtml(String(c.rate_percent)) : "";
  ...
  const unsub = MARKETING_TYPES.includes(type) && cl && cl.id && cl.comms_token
    ? `<p style="margin:24px 0 0;color:#6b7280;font-size:12px;">Prefer not to get emails like this? <a href="${FN_BASE}/unsubscribe?c=${escapeHtml(String(cl.id))}&t=${escapeHtml(String(cl.comms_token))}" style="color:#6b7280;">Unsubscribe</a>.</p>`
    : "";
  ...
  if (type === "lead_ack") {
    ...
    const subject = `Thank you for your enquiry – ${companyText}`;
  ...
  if (type === "welcome") {
    const subject = `Welcome to ${companyText} – here's what happens next`;
  ...
  if (type === "factfind") {
    ...
    const subject = `${companyText} – your mortgage fact find (5–10 minutes)`;
  ...
  if (type === "docs_request") {
    ...
    if (outstanding.length) {
      ...
      const subject = chase
        ? `${companyText} – still waiting on ${n} document${n === 1 ? "" : "s"}`
        : `${companyText} – documents we'll need from you`;
      const opening = chase
    ...
    const subject = `${companyText} – documents we'll need from you`;
    const inner = `<p>Hi ${first},</p><p>To get your mortgage moving we'll need a few documents. Could you reply to this email attaching:</p><ul>${docs.map((d) => `<li>${escapeHtml(d.trim())}</li>`).join("")}</ul><p>Photos or scans are both fine. The sooner we have these, the sooner we can secure your deal${phone ? " – any questions, just reply" + phone : ""}.</p>${signoff}`;
    return { subject, html: wrap(inner, company) };
  }
  if (type === "submitted_update") {
    const subject = `Good news – your mortgage application has been submitted`;
    const inner = `<p>Hi ${first},</p><p>A quick update: your application${lender ? ` to <strong>${lender}</strong>` : ""}${forProp} has been submitted. 🎉</p><p>The lender will now assess it – typically they come back to us within a few working days, sometimes with requests for extra information (that's completely normal). We'll handle all of that and let you know the moment there's news.</p><p>Nothing is needed from you right now. Any questions, just reply${phone}.</p>${signoff}`;
    return { subject, html: wrap(inner, company) };
  }
  ...
  if (type === "birthday_greeting") {
    const subject = `Happy birthday from ${companyText}! 🎂`;
  ...
  if (type === "review_request") {
    ...
    const subject = reminder
      ...
      : `Thanks for choosing ${companyText} – how did we do?`;
  ...
  if (type === "fee_request") {
    ...
    const subject = `${companyText} – broker fee payment details`;
...
Deno.serve(async (req: Request) => {
  ...
  const results: any = { queued: null, comms: null, sent: 0, failed: 0, scoped, skipped_optout: 0, skipped_promos: 0, requeued: 0 }; // v19: skipped_optout counter · v20: skipped_promos counter · v21: requeued (Resend 429/5xx, row put back)
  ...
  if (!scoped) {
    ...
    const { data: extra, error: xErr } = await supabase.rpc("queue_comms_extras");
    if (xErr) results.comms_error = xErr.message; else results.comms = extra;   // v21 (fix 6)
  ...
  const { data: candidates, error: cErr } = await candQ;
  // v21 (fix 6): a failed candidate read must not look like an empty queue.
  if (cErr) return new Response(JSON.stringify({ ...results, error: cErr.message }), { status: 500, headers: { ...CORS, "Content-Type": "application/json" } });
  ...
  for (const e of emails ?? []) {
    ...
    /* v21 (fix 1): BOTH opt-out flags. marketing_opt_out is the one staff set on the client record;
       comms_optout is the one the unsubscribe link sets. Either one is the client saying no. */
    if (MARKETING_TYPES.includes(e.email_type) && e.client_id) {
      let optout = !!(e.clients && (e.clients.comms_optout || e.clients.marketing_opt_out));
      const { data: oc } = await supabase.from("clients").select("comms_optout,marketing_opt_out").eq("id", e.client_id).maybeSingle();
      if (oc) optout = !!(oc.comms_optout || oc.marketing_opt_out);
      if (optout) {
    ...
    if (FIN_PROMO_TYPES.includes(e.email_type) && String(s.financial_promotions_approved ?? "off").trim().toLowerCase() !== "on") {   // v21 (fix 8): normalised like the app
    ...
    const extra: any = {};
    let accepted: { subject: string; html: string } | null = null;
    try {
      ...
      if (e.email_type === "docs_request" && c?.id) {
        ...
        if (outstanding && outstanding.length) {
          ...
          const { count: priorDocs } = await supabase.from("email_queue")
            ...
            .eq("status", "sent").lt("created_at", e.created_at);   // v21 (fix 7): only a SENT request makes this a chase
      ...
      if (e.email_type === "review_request" && c?.id) {
        const { count: priorReviews } = await supabase.from("email_queue")
          ...
          .eq("status", "sent").lt("created_at", e.created_at);   // v21 (fix 7): only a SENT request makes this a reminder
      ...
      /* v21 (+): RFC 8058 one-click unsubscribe headers on the marketing-adjacent types, pointing
         at the same URL as the footer link. unsubscribe v2 performs the opt-out on POST, so a mail
         client's "Unsubscribe" button works and a link-prefetching scanner (GET) changes nothing. */
      const unsubUrl = MARKETING_TYPES.includes(e.email_type) && cl.id && cl.comms_token
        ? `${FN_BASE}/unsubscribe?c=${encodeURIComponent(String(cl.id))}&t=${encodeURIComponent(String(cl.comms_token))}`
        : "";
      const mailHeaders = unsubUrl ? { "List-Unsubscribe": `<${unsubUrl}>`, "List-Unsubscribe-Post": "List-Unsubscribe=One-Click" } : undefined;
      /* v21 (fix 4): the Idempotency-Key is the queue row id, so a retry of a row whose first
         attempt DID leave (function killed between fetch and stamp, reclaimed 10 min later) is
         de-duplicated by Resend instead of delivered twice. */
      const resp = await fetch("https://api.resend.com/emails", {
        method: "POST",
        headers: { Authorization: `Bearer ${apiKey}`, "Content-Type": "application/json", "Idempotency-Key": `email-queue/${e.id}` },
        body: JSON.stringify({ from: fromFor(adv), to: [to], reply_to: replyFor(adv), subject, html, ...(mailHeaders ? { headers: mailHeaders } : {}), ...(attachments ? { attachments } : {}) }),
      });
      let body: any = null;
      try { body = await resp.json(); } catch (_e) { body = null; }
      if (!resp.ok) {
        /* v21 (fix 4): rate-limited or Resend-side failure — nothing left, so put the row BACK
           rather than closing it as failed. The next run (cron or scoped) picks it up again. */
        if (resp.status === 429 || resp.status >= 500) {
          await supabase.from("email_queue").update({ status: "queued", claimed_at: null, error: `Resend ${resp.status} – will retry: ${body?.message || ""}`.trim() }).eq("id", e.id);
          results.requeued++; continue;
        }
        throw new Error(body?.message || `Resend error ${resp.status}`);
      }
      accepted = { subject, html };
    } catch (err) {
      await supabase.from("email_queue").update({ status: "failed", error: String(err) }).eq("id", e.id);
      results.failed++;
      continue;
    }
    /* v21 (fix 4): everything below runs AFTER Resend has accepted and OUTSIDE the failure catch.
       A DB hiccup here must never mark a delivered email 'failed' (a failed row invites a retry
       of mail that has already gone). Each stamp is best-effort; a stamping error is recorded on
       the response, not on the row. */
    try {
      const { subject, html } = accepted!;
      const { error: stampErr } = await supabase.from("email_queue").update({ status: "sent", sent_at: new Date().toISOString(), to_email: to, subject, body_html: html }).eq("id", e.id);
      /* A row that stays 'sending' here is reclaimed by the next run (10 min) and re-sent — and
         the Idempotency-Key above makes that re-send a no-op at Resend. Say so in the response. */
      if (stampErr) throw new Error(`sent but not stamped: ${stampErr.message}`);
      ...
      (results.stamp_errors ??= []).push({ id: e.id, error: String(err) });
    }
    results.sent++;
  }
  return new Response(JSON.stringify(results), { headers: { ...CORS, "Content-Type": "application/json" } });
